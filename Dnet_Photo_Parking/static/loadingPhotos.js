...
function search(){
    ...
    console.log(data);
    document.getElementById('inst').innerHTML += ('<tr><td>일시</td><td>차량번호</td><td>적발횟수</td><td>사진보기</td></tr>');
    for(i in data){
        console.log(i)
        document.getElementById('inst').innerHTML += ('<tr><td>'+data[i][1]+'</td><td>'+data[i][2]+'</td><td>'+data[i][4]+'</td><td> <button onClick="photo(this.id)" class="buletxt" style="cursor:pointer;" id="'+data[i][3]+'"</td>사진보기</tr>');
